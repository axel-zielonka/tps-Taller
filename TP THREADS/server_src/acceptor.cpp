...
Acceptor::Acceptor(const char* servname, Queue<Mensaje>& q, ListaQueues& l):
...
void Acceptor::run() {
    ...
    while (aceptando_clientes) {
        try {
            ...
            ClientThread* client = new ClientThread(std::move(peer), queue_juego, id);
...
void Acceptor::eliminar_cliente(ClientThread* client) {
...
void Acceptor::recolectar() {
    clients.remove_if([this](ClientThread* c) {
        if (c->is_dead()) {
            eliminar_cliente(c);
            return true;
        }
